...
class BreadcrumbFilter extends Filter
{
    ...
    public function execute($filterChain)
    {
        ...
        if ($loaded==null) {
            $loaded=true; // no sense doing this more than once

            global $xoopsTpl;

            /* Build a simple breadcrumb array
             *
             * Normally, this array would be a natural pull from the
             * application, but this is a collection of various demos
             * so it isn't very elegant.
             *
             * The important part is we can take some actions for
             * every page using a filter, including assigning Smarty
             * variables.
             *
             */
            ...
            if ($cur==$this->config()->get('DEFAULT_ACTION')) {
                ...
                $bcitems[]=array('caption'=>'Home', 'link'=> $this->controller()->getControllerPath($this->config()->get('DEFAULT_UNIT'), $this->config()->get('DEFAULT_ACTION')));
                switch ($cur) {
                    case 'HelloWorld':
                    ...
                    case 'SecurePage2':
                    case 'SecurePage3':
                        $bcitems[]=array('caption'=>'Secure Menu', 'link'=> $this->controller()->getControllerPath('App', 'Secure'));
                    ...
                    case 'TodoStartWork':
                    case 'TodoEndWork':
                        $bcitems[]=array('caption'=>'Todo List', 'link'=> $this->controller()->getControllerPath('App', 'TodoList'));
